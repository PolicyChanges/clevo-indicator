...
#define EC_SC 0x66 // embedded controller send command
#define EC_DATA 0x62 // embedded controler data port
...
static int ui_update(int* user_data);
static void ui_command_set_fan(long fan_duty);
static void ui_command_quit(char* command);
...
{
    printf("Simple fan control utility for Clevo laptops. Press Q to exit.\n");
    ...
    if (argc <= 1) {
        ...
        int ramp_interval = 9000;
        int duty = 20;
		bool is_shorthand = false;
		size_t index = 0;
		
		std::string cl_arguments;
		for(int i=1;i<argc;i++) cl_arguments.append(argv[i]).append(" ");
		//std::vector<std::string> options = {"--help", "-h", "--interval", "-i", "--set-duty", "-d", "--set-interval", "-v"};
	 
		if ( cl_arguments.contains("-h") || cl_arguments.contains("--help")) {
			printf(help_text);
			return main_dump_fan();
		} 
		if (((index = cl_arguments.find("-i")) != std::string::npos) || ((index = cl_arguments.find("--immediate")) != std::string::npos))
		{
			if(argc < 3){printf("not enough arguments\n");exit(EXIT_FAILURE);}
			std::string option_duty;
			std::string option_values = cl_arguments.substr(index);
			std::istringstream option_stream(option_values);
			std::getline(option_stream, option_duty, ' ');
			std::getline(option_stream, option_duty, ' ');
			
			ramp_interval = 0;
			duty = atoi(option_duty.c_str());
			if(duty < 0 || duty > 100) {printf("invalid fan duty %d!\n",duty); return EXIT_FAILURE;}
			//return printf("return ramp_duty2(%d, %d);", duty, ramp_interval);
			return ramp_duty2(duty, ramp_interval);
		}
		...
		{
			if(argc < 3){printf("not enough arguments\n");exit(EXIT_FAILURE);}
			std::string option_interval;
			std::string option_values = cl_arguments.substr(index);
			std::istringstream option_stream(option_values);
			std::getline(option_stream, option_interval, ' ');
			std::getline(option_stream, option_interval, ' ');
			
			ramp_interval = atoi(option_interval.c_str());
		}
		if(((index = cl_arguments.find("-d")) != std::string::npos) || ((index = cl_arguments.find("--set-duty")) != std::string::npos)) 
		{
			if(argc < 3){printf("not enough arguments\n");exit(EXIT_FAILURE);}
			std::string option_duty;
			std::string option_values = cl_arguments.substr(index);
...
int ramp_duty2(int val, int interval)
{
    clock_t ramp_time_ms = interval;
    clock_t delta;


    int previous_duty, next_duty;
    previous_duty = ec_query_fan_duty(EC_REG_CPU_FAN_DUTY);
    next_duty = val;
	if(previous_duty == next_duty) {
		printf("fan duty unchanged\n");
		main_dump_fan();
		return EXIT_SUCCESS;
...
#define DEBUG
        int new_duty =  previous_duty + (ramp_duty_percent * (next_duty - previous_duty));
       ...
       char s_time[256];
        get_time_string(s_time, 256, "%d/%m %H:%M:%S");
            printf("ramp_duty: %s CPU: %d°C, delta: %d, fan duty to %d%%\n",
            s_time, ec_query_cpu_temp(),
                 delta, new_duty);
#endif
...
// #define ALWAYS_AGGRESSIVE
       ...
       char s_time[256];
        get_time_string(s_time, 256, "%d/%m %H:%M:%S");
            printf("ramp_duty: %s CPU=%d°C, GPU1=%d°C, GPU2=%d°C auto fan duty to %d%%\n",
...
static int main_ec_worker(void)
{
       ...
       /* int new_fan_duty = share_info->manual_next_fan_duty;
        ...
        //if (share_info->auto_duty == 1) {
            ...
            next_duty = ec_auto_duty_adjust(nvidia_devices);   
                ...
                char s_time[256];
                get_time_string(s_time, 256, "%d/%m %H:%M:%S");
                printf("main_ec_worker: %s CPU=%d°C, GPU1=%d°C, GPU2=%d°C auto fan duty to %d\n",
                    s_time, share_info->cpu_temp, share_info->gpu_temp,
                    share_info->gpu_temp2, next_duty);
 #endif
                previous_duty
    ...
    printf("\nSetting to 40 percent fan duty. Exiting...\n");
    
    ec_write_fan_duty(40);
...
/*
    ...
    char *fan_curve[] =  {"Quiet", "Aggressive"};
    while (share_info->exit == 0) {
        char s_time[256];
        get_time_string(s_time, 256, "%d/%m %H:%M:%S");
        printf("main_ec_worker: %s Fan Mode: %s | CPU=%d°C | GPU1=%d°C | GPU2=%d°C | Auto fan duty to %d\n",
            s_time, fan_curve[share_info->aggressive_fan_curve], share_info->cpu_temp, share_info->gpu_temp,
            share_info->gpu_temp2, share_info->cpu_fan_duty);
        sleep(10);        
    }
...
static int main_dump_fan(void)
{
    ...
    printf("  CPU Temp: %d°C\n", ec_query_cpu_temp());
    if(nvidia_device_count == 2) {
    	printf("  GPU 1 Temp: %d°C\n", nvml_query_gpu_temp(nvidia_devices[0]));
        printf("  GPU 2 Temp: %d°C\n", nvml_query_gpu_temp(nvidia_devices[1]));
    } 
    if(nvidia_devices)
	    free(nvidia_devices);
    return EXIT_SUCCESS;
...
static void ui_command_quit(char
* command)
{
...
int ec_auto_duty_adjust(nvidia_device* nvidia_devices)
{
	...
	// TODO: REMOVE
	share_info->aggressive_fan_curve = 1;
